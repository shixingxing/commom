...
public final class Detector {
     ...
     */
    public static PDF417DetectorResult detect(BinaryBitmap image, Map<DecodeHintType, ?> hints,
    ...
    private static void copyToResult(ResultPoint[] result, ResultPoint[] tmpResult,
    ...
    private static ResultPoint[] findRowsWithPattern(BitMatrix matrix, int height, int width,
            int startRow, int startColumn, int[] pattern) {
        ...
        for (; startRow < height; startRow += ROW_STEP) {
            ...
            if (loc != null) {
                while (startRow > 0) {
                    int[] previousRowLoc = findGuardPattern(matrix, startColumn, --startRow, width,
        ...
        if (found) {
            ...
            for (; stopRow < height; stopRow++) {
                int[] loc = findGuardPattern(matrix, previousRowLoc[0], stopRow, width, false,
                        pattern, counters);
    ...
    private static float patternMatchVariance(int[] counters, int[] pattern,
        ...
        for (int x = 0; x < numCounters; x++) {
            ...
            float variance = counter > scaledPattern ? counter - scaledPattern : scaledPattern
                    - counter;
